...
Follow `docs/v1-plan/wiring-supabase-cloudflare` top to bottom. Phase 1 is `supabase link` + `supabase db push`. Phase 4 is `supabase secrets set …`. Phase 7 is `supabase functions deploy --no-verify-jwt` (per-function flag must match `config.toml`).
