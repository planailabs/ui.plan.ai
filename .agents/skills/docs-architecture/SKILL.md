...
│   │   ├── index.astro       # marketing/start page (build-time platform-status detection)
│   │   └── workbench/        # V1 workbench app (auth + review surface)
│   ├── layouts/Workbench.astro
│   └── lib/                  # browser-side Supabase client + auth helpers
├── public/                   # main app static
│   ├── _headers              # CF Pages headers (CSP, cache, /workbench no-store)
│   ├── _redirects            # CF Pages redirects (/docs → /docs/start-here/welcome/)
│   └── docs/                 # GENERATED by build:docs — gitignored
├── supabase/                 # V1 Supabase project (migrations + Edge Functions). NEVER `db push` from this repo without explicit user ask — see `supabase-setup` skill.
│   ├── config.toml
│   ├── migrations/*.sql      # timestamped (YYYYMMDDhhmmss_*)
│   ├── seed.sql
│   └── functions/
