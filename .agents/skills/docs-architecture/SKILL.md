...
| `tsconfig.json` | exclude `supabase/functions` | Deno Edge Functions are not typechecked by root Astro |
...
- Content schema is extended (`starlight/src/content.config.ts`) for custom keys: `stability` (`stable` | `working` | `experimental`), `last_synced_with` (string), `sources` (array of strings), `verify_against` (array of strings). Required by the docs content. Removing these is a breaking change.
...
- Supabase artifacts: migrations and seed under `supabase/`; Edge Functions under `supabase/functions/` and verified separately with the Supabase/Deno toolchain.
- Supabase operations: read `.agents/skills/supabase-setup/SKILL.md` before editing migrations, Edge Functions, or Supabase env/secret guidance.
